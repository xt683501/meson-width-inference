...
Check the downloads with `cd weights && sha256sum -c SHA256SUMS`. The model-ready training table, one central-mass row for each of 370 mesons, is [`data/meson-train.csv`](data/meson-train.csv). For more detailed measured particle properties, consult the [Particle Data Group](https://pdg.lbl.gov/); the model and study are described in the paper.
